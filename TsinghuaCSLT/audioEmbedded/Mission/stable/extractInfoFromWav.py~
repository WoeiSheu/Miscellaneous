...
def extractInfoWithMCLT(audio, BlockLen, infoLen):
    ...
    for n in range( 4096 ):
        smax = 0
        X = MCLT.FastMCLT(audio[0][n:n+BlockLen])
    ...
    offset = Loc[maxSameBits] - BlockLen/2
...
def test():
    ...
    #nchannels, sampwidth, framerate, nframes, wave_data, time = disposeWav.read_wave_data("/root/record1.wav")
    BlockLen = 4096
    infoLen = 9
